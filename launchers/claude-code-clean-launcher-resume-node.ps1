...
Write-Host ("  Batch File:  {0}\claude-code-clean-launcher-resume-node.bat" -f $PSScriptRoot)
Write-Host ("  Script File: {0}\claude-code-clean-launcher-resume-node.ps1" -f $PSScriptRoot)
Write-Host
...
# Check for win-claude-code wrapper
try {
  $null = Get-Command win-claude-code -ErrorAction Stop
  Write-Host "  Launcher: Using win-claude-code wrapper (Windows-optimized)"
}
catch {
  Write-Host
  Write-Host ('=' * 63) -ForegroundColor Red
  Write-Host "   ERROR: win-claude-code NOT FOUND" -ForegroundColor Red
  Write-Host ('=' * 63) -ForegroundColor Red
  Write-Host
  Write-Host "The win-claude-code command is not installed or not in PATH." -ForegroundColor Yellow
  Write-Host
  Write-Host "To install:" -ForegroundColor Cyan
  Write-Host "  npm install -g win-claude-code" -ForegroundColor White
  Write-Host
  Write-Host "Repository:" -ForegroundColor Cyan
  Write-Host "  https://github.com/somersby10ml/win-claude-code" -ForegroundColor White
  Write-Host
...
while ($continueRunning) {
  ...
  Write-Host ("  Command: win-claude-code --dangerously-skip-permissions -r")
  ...
  try {
    ...
    & win-claude-code --dangerously-skip-permissions -r
  ...
  Write-Host ('=' * 63) -ForegroundColor Cyan
  Write-Host
  Write-Host "  [R] Relaunch Claude Code (resume mode)" -ForegroundColor Green
