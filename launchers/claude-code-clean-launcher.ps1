...
Write-Header "CLAUDE CODE LAUNCHER (PATCH-1 ENHANCED)"

# Display launcher information
Write-Host "[LAUNCHER INFO]"
Write-Host ("  Batch File:  {0}\claude-code-clean-launcher.bat" -f $PSScriptRoot)
Write-Host ("  Script File: {0}\claude-code-clean-launcher.ps1" -f $PSScriptRoot)
Write-Host
...
try {
  ...
  Write-Host "  Launcher: Using win-claude-code wrapper (Node-based v2.0.27)"
} catch {
  Write-Host
...
while ($continueRunning) {
  ...
  Write-Host ("  Command: win-claude-code --dangerously-skip-permissions")
  ...
  Write-Host ('=' * 63) -ForegroundColor Cyan
  Write-Host
  Write-Host "  [R] Relaunch Claude Code in this directory" -ForegroundColor Green
