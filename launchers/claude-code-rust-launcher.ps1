...
Write-Header "CLAUDE CODE LAUNCHER (RUST NATIVE)"

# Display launcher information
Write-Host "[LAUNCHER INFO]"
Write-Host ("  Batch File:  {0}\claude-code-rust-launcher.bat" -f $PSScriptRoot)
Write-Host ("  Script File: {0}\claude-code-rust-launcher.ps1" -f $PSScriptRoot)
Write-Host
...
while ($continueRunning) {
  Write-Host "[LAUNCHING CLAUDE CODE (RUST)]"
  Write-Host ("  Executable: {0}" -f $rustClaude)
  Write-Host ("  Arguments:  --dangerously-skip-permissions")
  ...
  try {
    ...
    & $rustClaude --dangerously-skip-permissions
    $exitCode = $LASTEXITCODE

    Write-Host
    Write-Host ('=' * 63)
    Write-Host "   CLAUDE CODE SESSION ENDED (RUST)"
    Write-Host ('=' * 63)
  ...
  Write-Host ('=' * 63) -ForegroundColor Cyan
  Write-Host
  Write-Host "  [R] Relaunch Claude Code (Rust native)" -ForegroundColor Green
